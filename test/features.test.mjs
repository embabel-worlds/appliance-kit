...
describe('the public browser feature entry point', () => {
  ...
  it('C3 distinguishes unknown watch state from no watch and retains unsupported/error feedback', async () => {
    ...
    for (const status of [401, 403, 404]) {
      ...
      // An old appliance names the upgrade instead of the status and detail behind the missing endpoint.
      if (status === 404) assert.match(watch().textContent, /Could not list watches\. This appliance is older than this feature/)
      else assert.match(watch().textContent, new RegExp(`HTTP ${status}.*Watch backend detail`))
      assert.doesNotMatch(watch().textContent, /Watch this/)
    }
  })
  ...
  it('says an appliance is too old for API keys instead of showing an empty list', async () => {
    const services = {
    ...
    assert.match(container.textContent, /Could not list API keys\. This appliance is older than this feature/)
