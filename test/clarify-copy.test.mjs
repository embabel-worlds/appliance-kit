...
test('studio failures distinguish access, missing capability and connection recovery', () => {
  ...
  // An old appliance is the one failure with a user-facing fix, so it names the upgrade instead of
  // the status and server detail that only describe the missing endpoint.
  assert.match(f('unsupported', 404), /^Could not run the view\. This appliance is older than this feature — upgrade it to use this\.$/)
  assert.doesNotMatch(f('unsupported', 404), /HTTP|Server detail/)
  ...
  for (const kind of ['unauthorized', 'unreachable', 'failed', 'refused']) {
