...
function HandlerStudioBody({ draft, onDraftConsumed }) {
    ...
    const validateNow = useCallback(async () => {
        ...
        if (!active.current || generation !== validationGeneration.current || handle.getText().trim() !== source)
            return;
        if (!isOk(outcome)) {
            if (isAbsent(outcome))
    ...
    const dryRun = useCallback(async () => {
        ...
        if (!active.current || generation !== dryRunGeneration.current)
            return;
        setBusy(false);
        if (!isOk(outcome))
            return setRunStatus({ tone: 'error', text: failureMessage(outcome, 'dry-run this agent') });
