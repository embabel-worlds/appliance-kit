...
export function parseSurface(interfacesTs) {
 ...
 */
export function membersOf(surface, path) {
    if (!surface)
        return [];
    if (path.length === 0) {
        return [
 ...
 */
export function gatewayPathAt(before) {
...
export function methodAt(surface, path) {
    if (!surface)
        return null;
    if (path.length === 1)
