...
const targets_ts_1 = require("./targets.js");
...
function edgeProps(spec) {
    ...
    if (mode === 'judged') {
        ...
        if (intent)
            parts.push(`intent:'${(0, targets_ts_1.esc)(intent)}'`);
    }
    const ai = [];
    const steering = spec.ai ?? {};
    if (str(steering.hint).trim())
        ai.push(`hint:'${(0, targets_ts_1.esc)(str(steering.hint).trim())}'`);
    if (str(steering.model).trim())
        ai.push(`model:'${(0, targets_ts_1.esc)(str(steering.model).trim())}'`);
...
function whereParts(spec, alias) {
    const parts = [];
    const target = targets_ts_1.TARGETS[spec.target];
    // Membership, never CONTAINS: `tags` is a list.
    if (target.tags && str(spec.tag))
        parts.push(`'${(0, targets_ts_1.esc)(str(spec.tag))}' IN ${alias}.tags`);
...
function composeDocuments(spec) {
    ...
    return lines(mode === 'judged' ? '// judged retrieval: several LLM calls per anchor — an explicit choice, never a default' : null, `MATCH (:Concept {value:'${(0, targets_ts_1.esc)(seed)}'})-[r:RELEVANT_TO${edgeProps(spec)}]->(d:Document)`, where.length ? `WHERE ${where.join('\n  AND ')}` : null, `RETURN d.title AS title, ${evidence}`, `ORDER BY r.score DESC LIMIT ${limitOf(spec)}`);
}
function composeFiles(spec) {
    ...
    if (mode === 'mentions') {
        ...
        return lines(`MATCH (:Concept {value:'${(0, targets_ts_1.esc)(seed)}'})-[r:RELEVANT_TO {via:'keyword'}]->(f:File)`, `RETURN f.name AS name, f.dir AS dir, f.content AS excerpt, f.modifiedAt AS modified`, `ORDER BY modified DESC LIMIT ${limitOf(spec)}`);
    }
    const seed = str(spec.seed).trim() || 'your idea';
    return lines('// judged retrieval over file contents: several LLM calls per anchor', `MATCH (:Concept {value:'${(0, targets_ts_1.esc)(seed)}'})-[r:RELEVANT_TO${edgeProps(spec)}]->(f:File)`, `RETURN f.name AS name, f.dir AS dir, r.score AS score, r.snippet AS evidence`, `ORDER BY r.score DESC LIMIT ${limitOf(spec)}`);
}
function composeThreads(spec) {
    const anchor = targets_ts_1.TARGETS.threads.anchors[spec.anchor ?? 'topic'];
