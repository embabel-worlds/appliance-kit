...
const tour_ts_1 = require("./tour.js");
/** Does [dictionary] declare [target]? */
function supports(dictionary, target) {
 ...
 */
function fitness(tour, dictionary) {
    const missing = [];
    const deferred = [];
    const seen = new Set();
    ...
    for (const text of tour.requires) {
        try {
            consider((0, tour_ts_1.parseTarget)(text));
        }
        catch {
...
function refusal(tour, dictionary) {
