...
function CodingAgentsSurface({ services, host, }) {
    ...
    const load = (0, react_1.useCallback)(async () => {
        ...
        setProbeMessage(probe.ok || probe.kind === 'unsupported' ? '' : (0, chrome_tsx_1.failureMessage)(probe, 'check MCP status'));
        if (modeConfig.ok) {
            ...
            setModeStatus({ tone: 'error', text: (0, chrome_tsx_1.failureMessage)(modeConfig, 'load MCP mode settings') });
        }
    }, [services]);
    (0, react_1.useEffect)(() => {
        void load();
    ...
    async function chooseMode(next) {
        ...
        if (!result.ok) {
            setModeStatus({ tone: 'error', text: (0, chrome_tsx_1.failureMessage)(result, 'save MCP mode settings') });
            return;
        }
        setMode(next);
        setChangedMode(true);
