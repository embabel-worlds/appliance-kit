...
function failureMessage(outcome, action) {
    ...
    switch (outcome.kind) {
        // An old appliance is the one failure the user can actually fix, so say so and leave out the
        // status and server detail that only describe the missing endpoint.
        case 'unsupported':
            return `Could not ${action}. This appliance is older than this feature — upgrade it to use this.`;
        case 'unauthorized':
