...
exports.useRunProgress = useRunProgress;
const react_1 = require("react");
const events_ts_1 = require("../../../vc/events.js");
/** How many lines to keep. A wide fan-out can emit hundreds; the recent ones are the interesting ones. */
const MAX_LINES = 200;
function useRunProgress(subscribe) {
    ...
    const abort = (0, react_1.useRef)(null);
    const boundQueryId = (0, react_1.useRef)(null);
    ...
    const begin = (0, react_1.useCallback)((operationId) => {
        close();
        setLines([]);
        setLive(true);
        boundQueryId.current = null;
        ...
        const consume = (event) => {
            ...
            setLines((current) => {
                const line = {
                    ...
                    text: (0, events_ts_1.describeVcEvent)(event),
                    failed: event.type === 'producer.error' || event.type === 'query.rejected',
                };
                const next = [...current, line];
                return next.length > MAX_LINES ? next.slice(-MAX_LINES) : next;
            });
            if ((0, events_ts_1.isTerminal)(event)) {
                setLive(false);
                close();
            }
        };
