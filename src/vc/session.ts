...
function scopeRef(alias: string, name: string): string {
  return `(${alias}:\`$${name}\`)`
}

/** Rename a whole-word variable in pipeline clauses — the view inliner's move. */
function renameVar(stages: string[], from: string, to: string): string[] {
  if (from === to) return stages
  const re = new RegExp(`\\b${from}\\b`, 'g')
  return stages.map((s) => s.replace(re, to))
}

interface NodeRef { variable: string; label: string | null }

function nodeAtoms(pattern: string): NodeRef[] {
...
export function planLine(
  ...
  nextAutoName: string,
  findByName: (name: string) => SessionBinding | undefined,
  findByVariable: (variable: string) => SessionBinding | undefined,
): LinePlan {
...
function pipelineFor(consumed: SessionBinding[], clause: string): string[] {
 ...
 */
export function completeQuery(raw: string): { cypher: string; note?: string } {
...
export function pipelineText(stages: string[], returnClause: string | null): string {
  const body = [...stages]
