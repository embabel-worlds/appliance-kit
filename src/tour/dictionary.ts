...
import { parseTarget } from './tour.ts'
...
export function supports(dictionary: TourDictionary, target: TourTarget): TourSupport {
  const { kind, name } = target
  if (dictionary.dynamic?.includes(kind)) return 'deferred'
  switch (kind) {
    case 'panel':
...
const has = (
  dictionary: TourDictionary,
  pick: (panel: TourPanelEntry) => string[] | undefined,
  name: string,
): TourSupport => (Object.values(dictionary.panels).some((p) => pick(p)?.includes(name)) ? 'declared' : 'missing')
 ...
 */
export function fitness(tour: Tour, dictionary: TourDictionary): TourFitness {
  const missing: TourTarget[] = []
  const deferred: TourTarget[] = []
  const seen = new Set<string>()
  const consider = (target: TourTarget) => {
    if (seen.has(target.text)) return
    seen.add(target.text)
    const verdict = supports(dictionary, target)
    if (verdict === 'missing') missing.push(target)
    if (verdict === 'deferred') deferred.push(target)
  }
  for (const step of tour.steps) if (step.target) consider(step.target)
...
export function refusal(tour: Tour, dictionary: TourDictionary): string {
