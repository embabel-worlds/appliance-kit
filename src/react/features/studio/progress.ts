...
import { type VcEvent, describeVcEvent, isTerminal } from '../../../vc/events.ts'
...
export function useRunProgress(
  subscribe: (onEvent: (event: VcEvent) => void, signal: AbortSignal) => void,
): RunProgress {
  const [lines, setLines] = useState<ProgressLine[]>([])
  const [live, setLive] = useState(false)
  const [runId, setRunId] = useState<string | null>(null)
  const abort = useRef<AbortController | null>(null)
  const boundQueryId = useRef<string | null>(null)
  ...
  const begin = useCallback((operationId?: string) => {
    close()
    setLines([])
    setLive(true)
    boundQueryId.current = null
    ...
    const consume = (event: VcEvent) => {
      ...
      if (operationId) {
        ...
        if (event.queryId && event.queryId !== boundQueryId.current) return
      }
