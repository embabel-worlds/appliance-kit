...
export class TipRotation {
  ...
  constructor(private readonly options: TipRotationOptions) {
  ...
  async next(): Promise<Hint | undefined> {
    const outcome = await this.options.hints.random(this.seen(), this.options.surface)
    if (!isOk(outcome) || !outcome.value) return undefined
 ...
 */
export function renderTipCard(hint: Hint, options: TipCardOptions = {}): HTMLElement {
  ...
  if (options.renderBody) body.innerHTML = options.renderBody(hint.body ?? '')
  else body.textContent = hint.body ?? ''
